...
<div class="card mt-3" >
    <h4 class="text-center h4 font-weight-bold text-dark">Offline Sales Report By Month</h4>
    ...
    <div class="d-flex justify-content-between ">      
     ...
     <div class="ml-3">
       <?php
        $dbobj  =DB::connect();
        $sql = "SELECT count(inv_id), sum(inv_total), sum(inv_paid) FROM tbl_invoice WHERE MONTH(inv_date)= '$month' AND YEAR(inv_date) = '$year' AND inv_type='offline' ";
 ...
 <div class="m-3">
    <table id="ls_report" class="table responsive table-bordered">
      <thead class="thead-light">
       ...
       <tbody id="sum_data">
                <?php 
                $sql_data = "SELECT * FROM tbl_invoice inv JOIN tbl_reg_customer cus ON inv.cus_id = cus.cus_id WHERE MONTH(inv_date) ='$month' AND YEAR(inv_date) = '$year' AND inv_type='offline'";
