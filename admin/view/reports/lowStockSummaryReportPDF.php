...
if($category==""){
    $sql = "SELECT newsp_id,newsp_name,npcat_category,newsp_qty,newsp_rlevel FROM tbl_newspaper np JOIN tbl_newspaper_category cat ON np.npcat_id = cat.npcat_id AND np.newsp_qty < np.newsp_rlevel ";
    $output .= "<h4 class='text-center'>Low Stock Report of ".$date ." </h4>";
}else{
    $sql = "SELECT newsp_id,newsp_name,npcat_category,newsp_qty,newsp_rlevel FROM tbl_newspaper np JOIN tbl_newspaper_category cat ON np.npcat_id = cat.npcat_id AND np.npcat_id='$category' AND np.newsp_qty < np.newsp_rlevel";
