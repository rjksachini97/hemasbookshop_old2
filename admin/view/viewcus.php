...
<script>
 $(document).ready(function(){
    var dataTable = $("#tblviewcus").DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "lib/mod_customer.php?type=viewCus",
            "type": "POST"
        },
        "columns":[
          {"data":"0"},
    ...
    $("#tblviewcus tbody").on('click','a',function(){ // on command is dynmacally content  a- anker tag
        var type= $(this).attr('title');
        var data= dataTable.row($(this).parents('tr')).data(); //parents command using for select top dstas
        var cid = data[0];
        if(type=="Edit"){
            $("#rpanel").load("view/updatecus.php?cusid="+cid);
        }else if(type=="Delete"){
            swal({
            title:"Do you want to Remove this Customer?",
            text:"You are trying to Remove Customer:"+cid,
            icon:"warning",
            buttons:true,
            dangerMode:true
          }).then((willDelete)=>{
              if(willDelete){
                var url = "lib/mod_customer.php?type=deleteCus";
                $.ajax({
                  method:"POST",
                  url:url,
                  data:{cusid:cid},
