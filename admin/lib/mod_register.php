...
eg. EMP00001
*/
function getNewRegUserId(){
	$dbobj = DB::connect();
	$sql = "SELECT rusr_id FROM tbl_reg_users ORDER BY rusr_id DESC LIMIT 1;";
	...
	if($nor == 0){
		...
		$num = substr($lastid, 3);
		$num++;
		$newid = "REG".str_pad($num,5,"0",STR_PAD_LEFT);
	}
...
the tbl_employee
*/
function addNewRegUser(){
	$rusr_id = $_POST[""];
	$rusr_fname = $_POST["txtfirstname"];
	$rusr_lname = $_POST["txtlastname"];
	$rusr_email = $_POST["txtemail"];
	$rusr_pass = $_POST["txtpass"];
	$rusr_passrepeat = $_POST["txtrepeatpass"];

	$dbobj = DB::connect();//edit from here

	$sql = "INSERT INTO tbl_reg_users(emp_id,emp_title,emp_name,emp_dob,emp_gender,emp_address,emp_mobile,emp_email,	emp_nic,emp_doj) VALUES(?,?,?,?,?,?,?,?,?,?);";
